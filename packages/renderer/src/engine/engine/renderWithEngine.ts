/**
 * @le-relief/renderer – IG Engine Production Renderer
 *
...
import type { IGQueueItem, IGFormattedPayload } from "@le-relief/types";
...
export interface CarouselAssetResult {
  ...
  exportDir: string;
  /** Always "engine" — legacy renderer has been removed. */
  renderedBy: "engine";
}
...
export async function renderWithEngine(
  ...
  const postType = queueItem.igType ?? queueItem.postType;
  if (overflowWarnings.length > 0) {
    console.warn(
      `[engine] overflow warnings for ${queueItem.id} (${postType}):`,
      overflowWarnings,
    );
  }
  ...
  if (!isExportReady(post)) {
    ...
    console.error(`[engine] ✗ overflow gate BLOCKED export for ${queueItem.id}\n${report}`);
    throw new Error(
      `[engine] Export blocked for ${queueItem.id} — unresolved overflow in template "${post.templateId}". ` +
  ...
  console.log(
    `[engine] ✓ rendered ${slidePaths.length} slides for ${queueItem.id}` +
    ` (postType="${postType}" → template="${post.templateId}")`,
  );

  return { mode: "rendered", slidePaths, payloadPath, fitReportPath, exportDir, renderedBy: "engine" };
}
