...
export async function GET(request: NextRequest) {
  try {
    ...
    const queries = await analyticsRepo.getTopSearchQueries(
      startDate,
      endDate,
      language || undefined,
      take + skip
    ...
    const paginatedQueries = queries.slice(skip, skip + take);
    ...
    return NextResponse.json(
      {
        ...
        queries: paginatedQueries.map((q) => ({
          query: q.query,
          count: q.count,
          zeroResultCount: q.zeroResultCount,
          zeroResultRate:
            q.count > 0 ? ((q.zeroResultCount / q.count) * 100).toFixed(1) : "0",
        })),
      },
      { status: 200 }
    );
