  ...
  for (const article of frArticles) {
    ...
    try {
      ...
      await createdRef.set({
        ...
        contentType,
        status: "published",
        featured: Boolean((article as ArticleRecord).featured),
        isBreaking: Boolean((article as ArticleRecord).isBreaking),
        isHomepagePinned: Boolean((article as ArticleRecord).isHomepagePinned),
        priorityLevel: (article as ArticleRecord).priorityLevel ?? null,
        language: "en",
        ...
        translatedAt: now,
        publishedAt: (article as ArticleRecord).publishedAt ?? now,
        scheduledAt: null,
        views: 0,
        createdAt: now,
        updatedAt: now,
